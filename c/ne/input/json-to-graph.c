...
static void AddNodeFromEntry(NodeContainer *nc, json_value *val, size_t context, time_t now){
	...
	Node* existing = FindNode(nc, name, len, NodeAt(nc, context));
	if (existing)
		touch_node(existing, activation, now);
	else
		AddNodeEx(nc, name, len, activation, NODE_INIT_WGHT + ( weight - NODE_INIT_WGHT ) * NODE_GUESS_WEIGHT_RELEVANCE , 1, context, 0, now);

}

static void AddNodesFromEntry(NodeContainer *nc, json_object_entry* entry, size_t context){
	...
	for (size_t i = 0; i < entry->value->u.array.length; i++){
		AddNodeFromEntry(nc, entry->value->u.array.values[i], context, now);
	}

}

static _Bool ProcessArrayLinkage(NodeContainer *nc, json_value *entry, double weight, double activation, size_t context, time_t now){
	...
	Node* A = FindNode(nc, a->u.string.ptr, a->u.string.length, NodeAt(nc, context));
	if (!A) return 0;
	A->lastTouched = change_time_now();
	Node* B = FindNode(nc, b->u.string.ptr, b->u.string.length, NodeAt(nc, context));
	if (!B) return 0;
...
static void AddConnectionFromEntry(NodeContainer *nc, json_value* val, size_t context, time_t now){
	if (!val) return;
	if (val->type != json_object) return;
	...
	char *graph = SeriliazeGraph(nc);
	dump_to_file(DEFAULT_DUMP_DIRECTORY "debug-response.txt", graph, strlen(graph));
	ProcessArrayLinkage(nc, arr, weight, activation, context, now);

}

static void AddConnectionsFromEntry(NodeContainer *nc, json_object_entry* entry, size_t context){
	...
	for (size_t i = 0; i < entry->value->u.array.length; i++)
		// Adding a connection
		AddConnectionFromEntry(nc, entry->value->u.array.values[i], context, now);

}

_Bool AddContextNodesFromJSON(char* context_name, size_t context_size, json_value* root, NodeContainer *nc){
	change_assert(root, "No context data provided. Json entry.\n");

	change_assert(root->type == json_object, "Context provided, but is not an object \n");

	Node* context = FindNodeGlobal(nc, context_name, context_size, CONTEXT_COUNT + 1);
	change_assert(context, "Context not found. Be carefull, there is also an edge case where the context exists but is not withing the first CONTEXT_COUNT nodes.\n");
	...
	for (size_t i = 0; i < root->u.object.length; i++){
		...
		if (strcmp(entry.name, "nodes") == 0) AddNodesFromEntry(nc, &entry, context_id);
		else if(strcmp(entry.name, "connections") == 0) AddConnectionsFromEntry(nc, &entry, context_id);
	}
	...
	for (size_t i = 0; i < doc->u.object.length; i++){
		...
		if (strcmp(entry.name, "nodes") == 0){
			...
			for (size_t j = 0; j < entry.value->u.array.length; j++){
				...
				AddNodeEx(nc, name.p, name.len, activation, weight, has_parent, (size_t)parent, 1, now);
			...
			for (size_t j = 0; j < entry.value->u.array.length; j++){
				...
				Node *A = NodeAt(nc, (size_t)A_id);
				Node *B = NodeAt(nc, (size_t)B_id);
				...
				BiLinkEx(nc, A, B, activation, weight);
			}
		}
	}
